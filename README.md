...
- Le date e gli stati vengono salvati in cache (`data/seen_items.json`) per
  evitare di richiamare il portale per gli stessi atti ogni volta.
...
| `/controlla` | Forza un controllo immediato di nuovi atti e nuove news |
...
- **Scheduling**: ogni 6 ore (00:00, 06:00, 12:00, 18:00 UTC)
- **Durata massima per run**: ~355 minuti — il bot resta attivo e in
  ascolto dei comandi Telegram per quasi tutta la finestra fino al
  prossimo trigger automatico
- **Ogni ciclo controlla sia l'Albo Pretorio sia le News** nello
  stesso job, alla stessa cadenza — nessun workflow separato, nessun
  costo aggiuntivo di minuti GHA
- **Persistenza dati**: il bot stesso esegue `git commit` + `git push`
  dei file di stato ogni volta che li aggiorna (non solo a fine job),
  così i dati non si perdono anche se il job viene interrotto per
  raggiunto limite di tempo
...
| File | Descrizione |
|---|---|
| `data/seen_items.json` | Cache atti albo: hash, date pubblicazione/scadenza, stato |
...
| `data/last_check.txt` | Timestamp dell'ultimo controllo (evita la disattivazione automatica del workflow per inattività dopo 60 giorni) |
...
La chiave va impostata come secret del repository:

| Secret | Descrizione |
|---|---|
| `STATE_ENCRYPTION_KEY` | Chiave di cifratura per i file con dati personali. Generarla con: `python3 -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"` |
