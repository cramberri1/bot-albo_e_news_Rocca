...
async def _run_check_albo(bot: Bot) -> dict:
    ...
    work_items = new_items + pending_items

    if work_items:
        log.info(
            f"🆕 {len(new_items)} nuovi atti, {len(pending_items)} con consegne da ritentare "
            "— recupero PDF..."
        )
        async with httpx.AsyncClient(follow_redirects=True, timeout=60) as client:
            session_url = await open_session(client)
            if not session_url:
                ...
                return {"ok": False, "new": 0, "total": len(seen)}
            await enrich_with_pdf(client, session_url, work_items, push_cache=False)
