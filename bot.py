        ...
        for repo_root, relative_paths in grouped.items():
            ...
            for attempt in range(1, max_retries + 1):
                ...
                if fetch.returncode != 0:
                    log.warning(
                        f"git fetch origin/{target_branch} fallito in {repo}: "
                        f"{_safe_error(fetch.stderr)}"
                    )
                    all_ok = False
                    break
...
async def fetch_all_pages(
    ...
    diagnostics.setdefault("skipped_cards", 0)
    ...
    r.raise_for_status()
    page_items = parse_albo_html(r.text, diagnostics=diagnostics, page_number=1)
    diagnostics["pages_read"].append(1)
    ...
    while has_next_page(r.text):
        ...
        r.raise_for_status()
        page_items = parse_albo_html(r.text, diagnostics=diagnostics, page_number=page)
...
def parse_albo_html(
    ...
    *,
    ...
    for card in cards:
        ...
        if not title or not num_riga:
            ...
            if diagnostics is not None:
                diagnostics["skipped_cards"] = int(diagnostics.get("skipped_cards", 0)) + 1
                if page_number is not None and page_number not in diagnostics.setdefault("partial_pages", []):
                    diagnostics["partial_pages"].append(page_number)
            log.warning(
                "Card Albo ignorata perché incompleta "
                f"(pagina={page_number or '?'}, "
    ...
    if mc02_candidates and not items:
        if diagnostics is not None and page_number is not None:
            if page_number not in diagnostics.setdefault("unreadable_pages", []):
                diagnostics["unreadable_pages"].append(page_number)
            if page_number not in diagnostics.setdefault("partial_pages", []):
                diagnostics["partial_pages"].append(page_number)
        log.warning(
            f"Pagina {page_number or '?'} con {mc02_candidates} card MC02 ma nessun atto parsabile: "
            "la pagina viene saltata e la lettura prosegue in modalità parziale."
        )
...
async def cmd_atti(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Resta come rete di sicurezza per anomalie strutturali non recuperabili;
        # le singole pagine/card malformate vengono invece gestite fail-soft.
        log.warning(f"/atti: snapshot Albo strutturalmente anomalo: {_safe_error(error)}")
        await msg.edit_text(
            "⚠️ Ho recuperato il portale, ma la struttura generale dell'Albo non è interpretabile. "
            "Non posso fornire un elenco attendibile in questa lettura." + MENU_TEXT,
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    ...
    if items is None:
        ...
        return
    if not items:
        if diagnostics.get("partial_pages") or diagnostics.get("skipped_cards"):
            pages = diagnostics.get("unreadable_pages") or diagnostics.get("partial_pages") or []
            page_text = ", ".join(str(p) for p in sorted(set(pages))) if pages else "una o più pagine"
            await msg.edit_text(
                "⚠️ Il portale è raggiungibile, ma non sono riuscito a estrarre atti dalle "
                f"pagine problematiche ({page_text}). Non posso quindi garantire che l'Albo sia vuoto."
                + MENU_TEXT,
                parse_mode=ParseMode.MARKDOWN,
            )
        else:
    ...
    # Avvisa l'utente senza bloccare il comando quando Halley espone una
    # pagina/card non interpretabile. Gli atti validi vengono comunque mostrati.
    partial_pages = sorted(set(diagnostics.get("partial_pages", [])))
    unreadable_pages = sorted(set(diagnostics.get("unreadable_pages", [])))
    skipped_cards = int(diagnostics.get("skipped_cards", 0) or 0)
    if partial_pages or skipped_cards:
        details = []
        if unreadable_pages:
            details.append(
                "pagina/e completamente non parsabili: "
                + ", ".join(str(p) for p in unreadable_pages)
            )
        if skipped_cards:
            details.append(f"schede incomplete saltate: {skipped_cards}")
        await update.message.reply_text(
            "⚠️ *Lettura parziale dell'Albo*\n"
            f"Ho recuperato correttamente *{len(items)} atti*, ma Halley ha restituito "
            "alcuni elementi in un formato che il bot non riesce a interpretare.\n"
            + (" · ".join(details) + "\n" if details else "")
            + "Gli atti mostrati sono quelli letti correttamente; *potrebbero mancarne alcuni*. ",
            parse_mode=ParseMode.MARKDOWN,
        )
